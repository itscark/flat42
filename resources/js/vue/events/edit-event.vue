<template>
    <div>
        <form action="" method="patch" @submit.prevent="onSubmit"
            ...
            <div class="item-middle pl-4">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control"
                           ...
                           id="title"
                           v-model="form.title">
                    <div class="invalid-feedback" v-if="form.errors.has('title')"
                         v-text="form.errors.get('title')"></div>
                </div>
                <div class="form-group ">
                    <label for="body">Body</label>
                    <input class="form-control" type="text" name="body" id="body" v-model="form.body">
                    <div class="invalid-feedback" v-if="form.errors.has('body')" v-text="form.errors.get('body')"></div>
                </div>

                <div class="form-group ">
                    <label for="date">Date</label>
                    <input class="form-control" type="date" name="date" id="date" v-model="form.date">
                    <div class="invalid-feedback" v-if="form.errors.has('date')" v-text="form.errors.get('date')"></div>
                </div>
            </div>
            <div class="item-right">
                <a class="btn btn-outline-danger"
                   @click="hideEditing"><i class="fas fa-times"></i> Cancel
                </a>

                <button class="btn btn-outline-success mt-2" type="submit"><i class="fas fa-check"></i>
                    Save
                </button>
            </div>
        </form>
...
<script>
    export default {
        ...
        methods: {
            ...
            onSubmit() {
                this.form.patch('api/events/' + this.item.id)
                    .then((thatItem) => {
                        this.hideEditing();
